...
def _files(sources: Sequence[Photo]) -> list[tuple[str, tuple[str, bytes, str]]]:
    ...
    Все снимки уходят полем ``image`` — и один, и несколько. Имя ``image[]``
    Images API понимает, а стоящий перед ним шлюз — нет: на двух фото он
    отвечал ``unknown_parameter`` / ``invalid_value``, то есть «Половинки» и
    полароид не работали в бою вообще. Повторённое короткое имя — это тот же
    массив в терминах multipart, и путь у него ровно тот, что уже работает на
    одиночных приколах.

    Имена частей при этом нумеруются. Из MAX оба снимка приезжают под одним и
    тем же ``photo.jpg``, а про разбор внутри шлюза мы ничего не знаем.
    Одинаковые имена — единственное, чем он мог бы склеить две части в одну, и
    человек получил бы полароид, на котором он дважды взрослый. Одиночное фото
    остаётся без номера: этот запрос в бою работает, и менять в нём ради
    единообразия нечего.
    """
    if len(sources) == 1:
        first = sources[0]
