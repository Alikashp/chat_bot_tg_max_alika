...
class OpenAIImages:
    ...
    async def edit(
        ...
        *,
        ...
        Приходит он при этом из настроек прикола, а не только из общих: параметр
        понимает не всякая модель (``gpt-image-2`` отвечает на него
        ``invalid_input_fidelity_model``), а модель у прикола своя. None
        означает «бери настроенную у провайдера», пустая строка — «этой модели
        не посылать».
        ...
        fidelity = self._input_fidelity if input_fidelity is None else input_fidelity
        if fidelity:
            # Пустое значение выключает параметр целиком: его понимает не
            # всякая модель и не всякий шлюз к Images API, а неизвестное поле —
            # это 400 вместо картинки.
...
def _files(sources: Sequence[Photo]) -> list[tuple[str, tuple[str, bytes, str]]]:
    ...
    Одно фото уходит полем ``image``, несколько — полем ``image[]``. Именно эта
    форма работала в бою, и пробовать вместо неё повторённое ``image`` уже
    пришлось: шлюз отвечает на него ``duplicate_parameter``. Одиночный запрос
    трогать тем более нечего.
