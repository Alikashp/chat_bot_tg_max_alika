...
class Settings(BaseSettings):
    ...
    #: Отправлять ли чек вместе с платежом.
    #:
    #: Выключено по умолчанию, и это не осторожность ради осторожности. Чек
    #: уходит онлайн-кассе, а её сначала надо подключить в кабинете ЮKassa и
    #: настроить у оператора фискальных данных. Отправленный раньше времени
    #: чек — это ошибка на каждом платеже; неотправленный, когда касса уже
    #: ждёт, — штраф за каждый недоставленный. Поэтому переключатель, а не
    #: догадка по наличию ключей.
    receipts_enabled: bool = False

    #: Ставка НДС в чеке. Значение из справочника ЮKassa: 1 — без НДС,
    #: 2 — 0%, 3 — 10%, 4 — 20%, 5 — 10/110, 6 — 20/120.
    #:
    #: Умолчание «без НДС» подходит упрощёнке, но проверить его обязан
    #: бухгалтер: неверная ставка — это неверный фискальный документ, за
    #: который отвечает продавец, а не код.
    receipt_vat_code: Annotated[int, Field(ge=1, le=6)] = 1

    #: Признак предмета расчёта. Подписка на бота — услуга.
    receipt_payment_subject: str = "service"

    #: Признак способа расчёта. «full_payment» — полный расчёт: доступ
    #: открывается сразу и целиком, ничего не остаётся «в долг».
    #:
    #: Не «full_prepayment»: предоплата обязала бы нас потом выставить ещё и
    #: чек зачёта предоплаты, отдельным запросом и в отдельный момент. Такого
    #: механизма у нас нет, и заводить его ради формулировки незачем —
    #: услуга оказывается с первой же минуты.
    receipt_payment_mode: str = "full_payment"

    #: Система налогообложения из справочника ЮKassa. Нужна только тем, у кого
    #: их несколько. Пусто — поле в чек не попадает вовсе.
    receipt_tax_system_code: Annotated[int | None, Field(ge=1, le=6)] = None
