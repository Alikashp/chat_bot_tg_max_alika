...
class User:
    ...
    в дневном расходе: они не сгорают по суткам. Для картинок на бесплатном
    тарифе бонус — единственный источник: дневной нормы у них нет, а всё
    бесплатное приходит разово (регистрация, друг, канал).
    ...
    раз), и время (по нему считается потолок наград, если он включён).
    Дублировать её полем у пользователя значило бы завести второй источник
    ...
    #: Откуда человек пришёл: payload из ссылки ?start=... при регистрации,
    #: «direct» — если ссылки не было. См. core/sources.py. Пишется один раз
    #: и не перезаписывается: это ответ на вопрос «откуда он взялся», а не
    #: «где он был в последний раз».
    source: str = sources.DIRECT
    #: Куда слать фискальный чек (54-ФЗ). Спрашивается один раз, перед первой
    #: оплатой картой, и дальше переиспользуется — в том числе автосписаниями,
    #: где спросить уже не у кого. Пусто у всех, кто картой не платил.
    email: str | None = None
    bonus_messages: int = 0
    bonus_images: int = 0
    #: Когда человеку начислили разовый бонус за подписку на канал. Пусто —
    #: не начисляли. Отметка нужна затем, что бонус разовый: без неё его
    #: получали бы столько раз, сколько раз нажать кнопку, — достаточно
    #: отписаться и подписаться снова. Хранится время, а не «да/нет»: по
    #: нему видно, когда это было, и это ничего не стоит.
    channel_bonus_at: datetime | None = None
...
class Chat:
    ...
    messenger: MessengerKind
    chat_id: str
    #: Идентификатор называет человека, а не переписку.
    #:
    #: В Telegram у личной переписки номер совпадает с номером человека, и
    #: разницы нет. В MAX это разные числа: номер переписки приходит только во
    #: входящем обновлении, а у разговора, который начинаем мы — подтверждение
    #: оплаты, напоминание о списании, — входящего обновления нет вовсе. Взять
    #: его неоткуда, и подставлять вместо него номер человека нельзя: MAX
    #: примет запрос и отправит сообщение не туда.
    #:
    #: Поэтому адресата мы называем честно, а как обратиться к человеку без
    #: номера переписки — знает адаптер: в MAX для этого есть отдельное поле.
...
class IncomingMessage:
    ...
    #: Чем мессенджер упорядочил это сообщение среди остальных: номер
    #: сообщения в Telegram, время обновления в MAX.
    #:
    #: Нужен приколам из двух фото. Два снимка, присланные разом, приезжают
    #: двумя обновлениями и разбираются параллельно — какое из них доберётся
    #: до базы первым, решает случай. А порядок тут меняет результат:
    #: инструкция провайдеру ссылается на снимки по номерам, и переставленные
    #: местами лица — это уже другая картинка. Ноль означает «мессенджер не
    #: сказал»: тогда порядок остаётся тем, в каком снимки дошли.
    order_key: int = 0
    action: str | None = None
