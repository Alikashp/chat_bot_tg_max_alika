...
class User:
    ...
    bonus_images: int = 0
    #: Разборы документов, выданные разово. Устроено как у картинок и по той
    #: же причине: на бесплатном тарифе дневной нормы у них нет — разбор
    #: длинного файла стоит слишком дорого, чтобы возобновлять его сутками.
...
class IncomingMessage:
    ...
    #: Ссылка на присланный файл в терминах мессенджера. Ядро её не
    #: разбирает: отдаёт обратно адаптеру, чтобы тот скачал файл.
    document_ref: str | None = None
    #: Имя присланного файла. Без него формат не определить: docx и pptx
    #: внутри одинаковы, и различает их только расширение.
