...
def session_for(deps: Deps, user: User) -> Session:
    ...
    вебхуком провайдера, напоминание о списании — по расписанию. Входящего
    обновления в этот момент нет, а значит нет и номера переписки: собрать
    адресата можно только из самого человека.

    Поэтому чат помечается как «это человек, а не переписка». Пометка не
    формальность: в MAX номер переписки и номер человека — разные числа, и
    подставить одно вместо другого значит отправить сообщение не туда.
    """
    return Session(
        user=user,
        chat=Chat(
