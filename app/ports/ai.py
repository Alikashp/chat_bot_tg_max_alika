"""Порты провайдеров ИИ.

Ядро не знает ни имени провайдера, ни названия модели: модель приходит
параметром из конфига (§2.2 задания — модель не выбирается пользователем и
нигде не упоминается).
"""

from __future__ import annotations

from collections.abc import Sequence
from dataclasses import dataclass
from enum import StrEnum
from typing import Protocol

from app.core.models import ChatTurn, Photo


class ImageQuality(StrEnum):
    """Качество отрисовки.

    Пользователю не показывается и им не выбирается — это параметр тарифа
    в конфиге. Разница в цене между LOW и MEDIUM почти на порядок
    (docs/research.md §4.3), поэтому бесплатный тариф рисует в LOW.
    """

    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"


class ContentRefusedError(Exception):
    """Провайдер отказался работать с этим запросом по правилам содержания.

    Живёт в порту, а не в адаптере, потому что реагировать на такой отказ
    должно ядро: человеку надо сказать, что нарисовать это нельзя, а не
    «что-то пошло не так, попробуй ещё раз» — попыток тут сколько ни делай,
    ответ будет тот же.

    От сбоя провайдера отличается принципиально: повторять бессмысленно,
    предохранитель трогать нечестно (провайдер жив и ответил по существу), а
    лимит, как и при любой недоставке результата, не списывается.
    """


@dataclass(frozen=True, slots=True)
class Answer:
    """Ответ модели вместе с причиной, по которой она замолчала.

    Причина здесь не для отчётности. Упёршись в потолок длины, модель
    обрывает фразу на полуслове — и человек, не зная об этом, читает огрызок
    как законченный ответ. Признак ``truncated`` позволяет сказать честно и
    предложить дослушать.
    """

    text: str
    #: Ответ упёрся в потолок длины, а не закончился сам.
    truncated: bool = False
    #: Сколько токенов провайдер посчитал за запрос и за ответ. None —
    #: не сказал: не всякий шлюз к /chat/completions возвращает usage, а
    #: ноль вместо «неизвестно» испортил бы любую сумму по учёту.
    tokens_in: int | None = None
    tokens_out: int | None = None


class LLMProvider(Protocol):
    """Провайдер текстовых ответов."""

    async def complete(
        self, turns: Sequence[ChatTurn], *, model: str, max_tokens: int = 0
    ) -> Answer:
        """Возвращает ответ на диалог.

        ``max_tokens`` — потолок длины ответа; ноль означает «настроенный у
        провайдера». Явный нужен там, где работа заведомо длиннее реплики в
        чате: доклад на несколько страниц не помещается в потолок, рассчитанный
        на разговор, и обрывается на полуслове.

        Реализация обязана задать явный таймаут (§3.4.6). Исключения не
        глотает: решение «показать ошибку и не списать лимит» принимает
        вызывающий сценарий.
        """
        ...


class ImageProvider(Protocol):
    """Провайдер картинок."""

    async def generate(
        self, prompt: str, *, quality: ImageQuality, model: str = ""
    ) -> Photo:
        """Рисует картинку по текстовому описанию (§2.3).

        ``model`` пустая — берётся та, что настроена у провайдера. Явная
        нужна там, где модель выбирается не на весь сервис, а на конкретную
        работу.
        """
        ...

    async def edit(
        self,
        sources: Sequence[Photo],
        instruction: str,
        *,
        quality: ImageQuality,
        model: str = "",
        input_fidelity: str | None = None,
    ) -> Photo:
        """Переделывает присланные фото по инструкции (пресеты, §2.4).

        Фото может быть несколько: пресет «я и я в детстве» соединяет два
        снимка в один кадр. Порядок существенный — инструкция ссылается на
        снимки по номерам, и провайдер вытягивает детали первого сильнее,
        чем остальных.

        ``model`` пустая — берётся та, что настроена у провайдера.

        ``input_fidelity`` None — берётся настроенная у провайдера. Пустая
        строка выключает параметр для этого вызова: он поддерживается не
        всякой моделью, а неизвестное ей поле — это 400 вместо картинки.
        """
        ...
